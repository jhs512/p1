...
if (!arg) {
  console.error(
    "Usage: pnpm sync <series>/<id>   e.g.  pnpm sync 001-Java-Basic/001",
  );
  process.exit(1);
}
...
if (!matchEntry) {
  console.error(
    `❌  ${SERIES_DIR}/${episodeId}-*.tsx 파일을 찾을 수 없습니다.`,
  );
  process.exit(1);
}
...
function detectSpeechBounds(
  audioFile: string,
  totalSecs: number,
): { speechStartFrame: number; speechEndFrame: number } {
  ...
  const trailing = [...silences]
    .reverse()
    .find((s) => s.end === null || s.end >= totalSecs - 0.1);
...
): { splits: number[]; sentenceEndFrames: number[] } {
...
for (const [key, scene] of Object.entries(VIDEO_CONFIG)) {
  ...
  if (hashes[key] === newHash) {
    if (existingAudioConfig[key]) {
      ...
      if (
        ...
        audioConfig[key].speechEndFrame === 0
      ) {
        ...
        changed = true;
        console.log(
          `[skip] ${audio}  speechStartFrame → ${bounds.speechStartFrame}, speechEndFrame → ${bounds.speechEndFrame} (재측정)`,
        );
      } else {
  ...
  audioConfig[key] = {
    durationInFrames: Math.ceil(totalSecs * FPS) + SCENE_TAIL_FRAMES,
    narrationSplits: [],
    sentenceEndFrames: [],
    speechStartFrame: 0,
    speechEndFrame: 0,
  ...
  console.log(
    `       speechStartFrame → ${bounds.speechStartFrame} (${(bounds.speechStartFrame / FPS).toFixed(2)}s)`,
  );
  console.log(
    `       speechEndFrame   → ${bounds.speechEndFrame} (${(bounds.speechEndFrame / FPS).toFixed(2)}s)`,
  );

  // 4) narrationSplits / sentenceEndFrames
  const { splits, sentenceEndFrames } = detectSplits(
    `${PUBLIC_DIR}/${audio}`,
    narration.length,
  );
  if (splits.length > 0) {
    ...
    console.log(
      `       sentenceEndFrames  → [${sentenceEndFrames.join(", ")}]`,
    );
  }
...
const arg = process.argv[2];
if (!arg) {
  console.error("Usage: pnpm render <series>/<id>");
  process.exit(1);
}
