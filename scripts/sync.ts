...
function writeAudioConfig(config: Record<string, SceneAudioData>): void {
  ...
  const lines = Object.entries(config)
    .map(([k, v]) => {
      ...
      return `  ${k.padEnd(16)}: { durationInFrames: ${v.durationInFrames}, narrationSplits: [${v.narrationSplits.join(", ")}], sentenceEndFrames: [${v.sentenceEndFrames.join(", ")}], speechStartFrame: ${v.speechStartFrame}, speechEndFrame: ${v.speechEndFrame}, wordStartFrames: [${wfStr}], wordStartMs: [${wmsStr}] },`;
    })
    .join("\n");
  ...
  try {
    ...
    for (const line of raw.split("\n")) {
      ...
      if (m) {
        ...
        existingAudioConfig[m[1]] = { durationInFrames: parseInt(m[2]), narrationSplits: splits, sentenceEndFrames: ends, speechStartFrame: parseInt(m[5]), speechEndFrame: parseInt(m[6]), wordStartFrames: [], wordStartMs: [] };
        ...
        // wordStartMs 파싱
        const wmsIdx = line.indexOf("wordStartMs: ");
        if (wmsIdx !== -1) {
          const start = line.indexOf("[", wmsIdx);
          if (start !== -1) {
            let depth = 0, end = -1;
            for (let i = start; i < line.length; i++) {
              if (line[i] === "[") depth++;
            ...
            if (end !== -1) {
              try { existingAudioConfig[m[1]].wordStartMs = JSON.parse(line.slice(start, end + 1)) as number[][]; } catch { /* ignore */ }
            }
          }
        }
        continue;
...
for (const [key, scene] of Object.entries(VIDEO_CONFIG)) {
  ...
  // 5) wordStartFrames + wordStartMs (Word Boundary 직접 인덱스 매핑)
  if (wordBoundaries.length > 0) {
    const { displayWords: dwInfos } = buildGlobalAlignment(narration);
    ...
    for (let ri = 0; ri < resultFrames.length; ri++) {
      ...
      let lastF = firstValidF, lastM = firstValidM;
      for (let i = 0; i < rowF.length; i++) {
        if (rowF[i] === -1) { rowF[i] = lastF; rowM[i] = lastM; }
        else { lastF = rowF[i]; lastM = rowM[i]; }
      }
    }
