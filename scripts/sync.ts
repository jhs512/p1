...
import { loadMergedConfig } from "./config-cascade";
...
function resolveSeriesDir(parts: string[]): string {
  ...
  for (const part of parts) {
    ...
    if (existsSync(fullPath)) {
      ...
      const match = readdirSync(resolved).find(
        (d) => d.startsWith(part + "-") || d === part,
      );
      if (!match) {
        console.error(
          `❌  시리즈 폴더를 찾을 수 없습니다: ${part} (in ${resolved})`,
        );
        process.exit(1);
      }
...
const matchEntry = readdirSync(SERIES_DIR).find(
  (f) => f.startsWith(filePrefix + "-1-") && f.endsWith(".tsx"),
);
if (!matchEntry) {
  console.error(
    `❌  ${SERIES_DIR}/${filePrefix}-1-*.tsx 파일을 찾을 수 없습니다.`,
  );
  process.exit(1);
}
...
if (Object.keys(pronMap).length)
  console.log(`📖  발음 사전: ${Object.keys(pronMap).length}개`);
...
export function buildGlobalAlignment(narration: string[]): {
  globalTtsCount: number;
  displayWords: DisplayWordInfo[];
} {
...
type SceneAudioData = {
  ...
  wordTiming: Record<string, number[]>; // display 단어 → [첫 등장 프레임, 두 번째 등장 프레임, ...]
};
  ...
  const lines = Object.entries(config)
    .map(([k, v]) => {
      const wfStr = (v.wordStartFrames ?? [])
        .map((s) => `[${s.join(",")}]`)
        .join(",");
      const weStr = (v.wordEndFrames ?? [])
        .map((s) => `[${s.join(",")}]`)
        .join(",");
      const wmsStr = (v.wordStartMs ?? [])
        .map((s) => `[${s.join(",")}]`)
        .join(",");
      const wtStr = Object.entries(v.wordTiming ?? {})
        .map(
          ([w, frames]) =>
            `"${w.replace(/\\/g, "\\\\").replace(/"/g, '\\"')}": [${frames.join(",")}]`,
        )
        .join(", ");
      return `  ${k.padEnd(16)}: { durationInFrames: ${v.durationInFrames}, narrationSplits: [${v.narrationSplits.join(", ")}], sentenceEndFrames: [${v.sentenceEndFrames.join(", ")}], speechStartFrame: ${v.speechStartFrame}, speechEndFrame: ${v.speechEndFrame}, wordStartFrames: [${wfStr}], wordEndFrames: [${weStr}], wordStartMs: [${wmsStr}], wordTiming: { ${wtStr} } },`;
    })
    .join("\n");
...
for (const [key, scene] of Object.entries(VIDEO_CONFIG)) {
  ...
  try {
    wordBoundaries = JSON.parse(ttsRes.stdout.trim()) as WBEvent[];
  } catch {
    /* ignore */
  }
  ...
  console.log(
    `       실측 ${totalSecs.toFixed(2)}s  (${wordBoundaries.length}개 Word Boundary)`,
  );
  audioConfig[key] = {
  ...
  if (wordBoundaries.length > 0) {
    ...
    audioConfig[key].speechStartFrame = Math.round(
      (first.offset / TICKS) * FPS,
    );
    audioConfig[key].speechEndFrame = Math.round(
      ((last.offset + last.duration) / TICKS) * FPS,
    );
  } else {
    // fallback: 0 / totalFrames
    audioConfig[key].speechStartFrame = 0;
    audioConfig[key].speechEndFrame =
      audioConfig[key].durationInFrames - SCENE_TAIL_FRAMES;
  }
  console.log(
    `       speechStartFrame → ${audioConfig[key].speechStartFrame} (${(audioConfig[key].speechStartFrame / FPS).toFixed(2)}s)`,
  );
  console.log(
    `       speechEndFrame   → ${audioConfig[key].speechEndFrame} (${(audioConfig[key].speechEndFrame / FPS).toFixed(2)}s)`,
  );
  ...
  if (narration.length > 1 && wordBoundaries.length > 0) {
    ...
    for (let si = 0; si < narration.length - 1; si++) {
      ...
      const endIdx = Math.min(
        sentenceStartTtsIdx[si] + sentenceTtsCounts[si] - 1,
    ...
    const lastEndIdx = Math.min(
      sentenceStartTtsIdx[lastSi] + sentenceTtsCounts[lastSi] - 1,
      wordBoundaries.length - 1,
    );
    const lastWbFinal = wordBoundaries[Math.max(0, lastEndIdx)];
    ends.push(
      Math.round(((lastWbFinal.offset + lastWbFinal.duration) / TICKS) * FPS),
    );
    ...
    console.log(
      `       sentenceEndFrames  → [${ends.slice(0, -1).join(", ")}]`,
    );
  }
  ...
  if (wordBoundaries.length > 0) {
    ...
    for (const dw of dwInfos) {
      ...
      if (dw.ttsCount === 0) {
        ...
        const lastWbIdx = Math.min(
          dw.firstTtsIdx + dw.ttsCount - 1,
          wordBoundaries.length - 1,
        );
        const lastWb = wordBoundaries[lastWbIdx];
        frame = Math.round((wordBoundaries[wbIdx].offset / TICKS) * FPS);
        endFrame = Math.round(
          ((lastWb.offset + lastWb.duration) / TICKS) * FPS,
        );
    ...
    for (let ri = 0; ri < resultFrames.length; ri++) {
      ...
      const firstValidF =
        rowF.find((f) => f !== -1) ?? audioConfig[key].speechStartFrame;
      const firstValidM =
        rowM.find((m) => m !== -1) ??
        Math.round((audioConfig[key].speechStartFrame / FPS) * 1000);
      let lastF = firstValidF,
        lastE = firstValidF,
        lastM = firstValidM;
    ...
    console.log(
      `       wordStartFrames   → ${resultFrames.map((s) => `[${s.join(",")}]`).join(" | ")}`,
    );
    console.log(
      `       wordEndFrames     → ${resultEndFrames.map((s) => `[${s.join(",")}]`).join(" | ")}`,
    );
  } else {
...
for (const data of Object.values(audioConfig)) {
  if (!data.wordEndFrames || data.wordEndFrames.length === 0) {
    data.wordEndFrames = data.wordStartFrames.map((sentence, si) => {
      const sentEnd =
        si < data.sentenceEndFrames.length
...
// wordTiming 계산 — narration 단어 텍스트 → [프레임, ...] (여러 번 등장 시 배열)
// 후행 구두점 제거해서 "초기식," → "초기식" 으로 룩업 가능하게 한다.
for (const [key, data] of Object.entries(audioConfig)) {
  const narration = (VIDEO_CONFIG as Record<string, { narration?: string[] }>)[
    key
  ]?.narration;
  if (!narration || data.wordStartFrames.length === 0) {
    data.wordTiming = {};
    continue;
  }
  const timing: Record<string, number[]> = {};
  narration.forEach((sentence, si) => {
    tokenizeNarration(sentence).forEach((token, wi) => {
      const frame = data.wordStartFrames[si]?.[wi];
      if (frame === undefined) return;
      const word = token.display.replace(/[.,!?;:、。！？，。！？]+$/, "");
      (timing[word] ??= []).push(frame);
    });
  });
  if (JSON.stringify(data.wordTiming) !== JSON.stringify(timing)) {
    data.wordTiming = timing;
    changed = true;
  }
}
