    ...
    <>
      {parts.map((part, i) => {
        ...
        if (OPERATORS.includes(part))
          return <span key={i} style={{ color: C_OP }}>{part}</span>;
        if (/^\d/.test(part))
  ...
  return (
    <div style={{
      ...
      {visibleLines.map((l, i) => {
        ...
        if (!isLast) {
          return <div key={i} style={{ color: "#d4d4d4", opacity: 0.55 }}><ColorizedCode text={l.text} /></div>;
        }
        return <TypingLine key={i} text={l.text} startFrame={l.startFrame} cps={cps} />;
      })}
    </div>
  );
};
    ...
    <>
      <AbsoluteFill style={{ background: "#1e1e1e", opacity }}>
        ...
        <div style={{
          ...
          {INTRO_OPS.map((op, i) => {
            const appear = spring({ frame: frame - i * 6, fps, config: { damping: 14, stiffness: 140 }, durationInFrames: 30 });
            const sc = interpolate(appear, [0, 1], [0.3, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
            return (
              <div key={op} style={{
                width: 180, height: 180, borderRadius: 24,
                border: `3px solid ${C_OP}88`, background: `${C_OP}1a`,
      ...
      <AbsoluteFill style={{ background: "#1e1e1e", opacity }}>
        ...
        {frame >= s && (
          <CodeLines lines={[
            ...
            { text: "// 정수끼리 나누면 소수점은 버려집니다.", startFrame: split1 },
          ]} />
        )}
      </AbsoluteFill>
      <Subtitle sentences={cfg.narration} splits={cfg.narrationSplits} speechStart={s} />
...
const RemScene: React.FC = () => {
  ...
  // 나눗셈 시각화 등장
  const divAppear = spring({ frame: frame - s, fps, config: { damping: 14, stiffness: 140 }, durationInFrames: 30 });

  // split0: 몫/나머지 강조 등장
  const detailAppear = spring({ frame: frame - split0, fps, config: { damping: 12, stiffness: 160 }, durationInFrames: 25 });

  // split1: 활용 예시 등장
  const usageAppear = spring({ frame: frame - split1, fps, config: { damping: 14, stiffness: 140 }, durationInFrames: 25 });
      ...
      <AbsoluteFill style={{ background: "#1e1e1e", opacity }}>
        ...
        {frame >= split1 && (
          <div style={{
            ...
            <span style={{ fontFamily: uiFont, fontSize: 24, color: "#666", marginLeft: 4 }}>→ 짝수</span>
            ...
            <span style={{ fontFamily: uiFont, fontSize: 24, color: "#666", marginLeft: 4 }}>→ 홀수</span>
          </div>
        )}
      </AbsoluteFill>
      <Subtitle sentences={cfg.narration} splits={cfg.narrationSplits} speechStart={s} />
          ...
          <Sequence key={i} from={startFrom} durationInFrames={d - startFrom}>
            <div style={{
              ...
              {SUMMARY_LINES.slice(0, i + 1).map((text, j) =>
                j < i
                  ? <div key={j} style={{ opacity: 0.5, color: "#d4d4d4" }}><ColorizedCode text={text} /></div>
                  : <TypingLine key={j} text={text} startFrame={0} cps={SUMMARY_CPS} />
              )}
            </div>
          </Sequence>
        ))}
...
export const JavaOperators: React.FC = () => (
  <AbsoluteFill style={{ background: "#1e1e1e" }}>
    ...
    <Sequence from={fromValues[2]} durationInFrames={VIDEO_CONFIG.addSubScene.durationInFrames}>
      <AddSubScene />
    </Sequence>
    <Sequence from={fromValues[3]} durationInFrames={VIDEO_CONFIG.mulDivScene.durationInFrames}>
      <MulDivScene />
    </Sequence>
    <Sequence from={fromValues[4]} durationInFrames={VIDEO_CONFIG.remScene.durationInFrames}>
      <RemScene />
    </Sequence>
