  ...
  return (
    ...
    >
      ...
      >
        Java
        <br />
        <span style={{ color: "#4ec9b0" }}>Loops</span>
      </div>
      <div
        style={{
          ...monoStyle,
        ...
        <ContentArea>
          ...
          >
            ...
            >
              ...
              <span style={{ color: "#d4d4d4" }}>; </span>
              <span style={{ color: C_INC, fontWeight: 700 }}>update</span>
              <span style={{ color: "#d4d4d4" }}>) {"{"}</span>
            </div>
            {/* 실행코드 */}
            <div
...
const EXEC_STEPS = [
  { i: 0, condPass: true, label: "first run", output: ["0"] },
  { i: 1, condPass: true, label: "second run", output: ["0", "1"] },
  { i: 2, condPass: true, label: "third run", output: ["0", "1", "2"] },
  { i: 3, condPass: true, label: "fourth run", output: ["0", "1", "2", "3"] },
  {
    i: 4,
    condPass: true,
    label: "fifth run",
...
// ── SummaryScene ──────────────────────────────────────────────
const SUMMARY_ROWS = [
  { label: "initializer", color: C_INIT, desc: "sets up the loop variable" },
  { label: "condition", color: C_COND, desc: "run if true, stop if false" },
  { label: "update", color: C_INC, desc: "changes the variable each round" },
] as const;
