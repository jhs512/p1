...
const BoxMetaphorAnim: React.FC = () => {
  ...
  const NAME_TAG_START = AUDIO_CONFIG.intro.wordTiming["name"][0];
  const DROP_START = AUDIO_CONFIG.intro.wordTiming["value"][0];
  const EXTRACT_START = AUDIO_CONFIG.intro.wordTiming["use"][0];
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ color: C_TEAL }}>Variable</span>
    ...
    <>
      <AbsoluteFill style={{ background: BG, opacity }}>
        <ContentArea>
          ...
          <SceneTitle title="1. What Is a Variable?" />
          <BoxMetaphorAnim />
        </ContentArea>
      </AbsoluteFill>
      <Subtitle
  ...
  return (
    ...
    >
      ...
      >
        Java
        <br />
        <span style={{ color: C_TEAL }}>Variables</span>
      </div>
      <div
        style={{
          fontFamily: uiFont,
          ...
          {frame >= s && (
            ...
            >
              ...
              >
                ...
                {badge("← space", C_SPACE, phase >= 1 ? ann1 : 0)}
              </div>
...
const QuizScene: React.FC = () => {
  ...
  // "우측의 age는 값으로 해석해야 합니다" 발화 시 우측 age glow (등장 후 소멸)
  const REVEAL_SPLIT2 =
    REVEAL_START + AUDIO_CONFIG.interpretReveal.narrationSplits[1];
  const valueGlowIn = spring({
    frame: frame - REVEAL_SPLIT2,
    fps,
    config: { damping: 12, stiffness: 120 },
    durationInFrames: 60,
  });
  const GLOW_FADE_START = REVEAL_SPLIT2 + 50;
  const valueGlowOut = interpolate(
    frame,
    [GLOW_FADE_START, GLOW_FADE_START + 25],
    [0, 1],
    { extrapolateLeft: "clamp", extrapolateRight: "clamp" },
  );
  const valueGlow = valueGlowIn * (1 - valueGlowOut);
  ...
  return (
    <>
      <AbsoluteFill style={{ background: BG, opacity }}>
        <ContentArea>
          <SceneTitle title="5. Interpretation Quiz" />
          {/* 오디오: 문제 */}
