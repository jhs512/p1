...
export const VIDEO_CONFIG = {
  ...
  summaryScene: {
    ...
    narration: [
      "세 가지 논리 연산자를 정리했습니다.",
      "[&&(발음:AND)]는 모두 참, [||(발음:OR)]는 하나라도 참, [!(발음:NOT)]는 반전입니다.",
    ] as string[],
...
// ── 컴포넌트: BoolPill ────────────────────────────────────────
const BoolPill: React.FC<{ value: boolean; size?: number }> = ({ value, size = 52 }) => {
  ...
  return (
    <span style={{
      ...
      borderRadius: 12, padding: "6px 22px", display: "inline-block",
    }}>
      {value ? "true" : "false"}
    </span>
  );
...
const EvalRow: React.FC<{
  ...
  const appear = spring({ frame: frame - startFrame, fps, config: { damping: 12, stiffness: 140 }, durationInFrames: 22 });
  const sc     = interpolate(appear, [0, 1], [0.8, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const resultApp = spring({ frame: frame - startFrame - 15, fps, config: { damping: 11, stiffness: 120 }, durationInFrames: 20 });
  const resultSc  = interpolate(resultApp, [0, 1], [0.7, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  return (
    <div style={{
      ...
      {frame >= startFrame + 15 && (
        <>
          <span style={{ fontSize: 40, color: "#444", transform: `scale(${resultSc})` }}>→</span>
...
const ThumbnailScene: React.FC = () => (
  <AbsoluteFill style={{ background: "#0d0d1a", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 28 }}>
    <div style={{
      ...
      background: "radial-gradient(circle, rgba(229,192,123,0.10) 0%, transparent 70%)",
      top: "50%", left: "50%", transform: "translate(-50%, -50%)",
    }} />
    <div style={{ fontFamily: uiFont, fontSize: 26, fontWeight: 700, color: C_LOG, letterSpacing: 10, opacity: 0.8 }}>JAVA</div>
    <div style={{
      ...
      Java<br /><span style={{ color: C_LOG }}>논리 연산자</span>
    </div>
    {/* 세 연산자 */}
    <div style={{
      ...
      {[["&&", "AND"], ["||", "OR"], ["!", "NOT"]].map(([op, label]) => (
        <div key={op} style={{
          display: "flex", flexDirection: "column", alignItems: "center", gap: 8,
          background: `${C_LOG}18`, border: `2px solid ${C_LOG}55`,
          borderRadius: 20, padding: "22px 36px",
        }}>
          <span style={{ fontSize: 56, fontWeight: 900, color: C_LOG }}>{op}</span>
          <span style={{ fontFamily: uiFont, fontSize: 22, color: C_LOG, opacity: 0.7 }}>{label}</span>
        </div>
      ))}
    </div>
  </AbsoluteFill>
    ...
    <>
      <AbsoluteFill style={{ background: "#1e1e1e", opacity }}>
        ...
        <div style={{
          ...
          {INTRO_OPS.map((op, i) => {
            const appear = spring({ frame: frame - i * 8, fps, config: { damping: 13, stiffness: 140 }, durationInFrames: 30 });
            const sc = interpolate(appear, [0, 1], [0.3, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
            return (
              <div key={op} style={{
                width: i === 2 ? 140 : 200, height: 200, borderRadius: 28,
                border: `3px solid ${C_LOG}88`, background: `${C_LOG}18`,
                boxShadow: `0 0 36px ${C_LOG}22`,
                display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10,
                transform: `scale(${sc})`, opacity: appear,
              }}>
                <span style={{ fontFamily: monoFont, fontFeatureSettings: MONO_NO_LIGA, fontSize: 64, fontWeight: 700, color: C_LOG }}>{op}</span>
                <span style={{ fontFamily: uiFont, fontSize: 22, color: C_LOG, opacity: 0.7 }}>
                  {op === "&&" ? "AND" : op === "||" ? "OR" : "NOT"}
                </span>
              </div>
            );
      ...
      <AbsoluteFill style={{ background: "#1e1e1e", opacity }}>
        ...
        {frame >= s && (
          <div style={{ position: "absolute", top: "44%", left: "50%", transform: "translate(-50%, -50%)" }}>
            <EvalRow left={true} op="&&" right={false} result={false} startFrame={s} dim={frame >= split0} />
          </div>
        )}
        ...
        {frame >= split0 && (
          <div style={{ position: "absolute", top: "64%", left: "50%", transform: "translate(-50%, -50%)" }}>
            <EvalRow left={true} op="&&" right={true} result={true} startFrame={split0} />
          </div>
        )}
      </AbsoluteFill>
    ...
    <>
      <AbsoluteFill style={{ background: "#1e1e1e", opacity }}>
        ...
        <div style={{
          ...
          transform: "translate(-50%, -50%)",
          display: "flex", flexDirection: "column", gap: 22, width: 900,
        }}>
          {SUMMARY_ROWS.map((row, i) => {
            const appear = spring({ frame: frame - i * 10, fps, config: { damping: 13, stiffness: 140 }, durationInFrames: 26 });
            const sc     = interpolate(appear, [0, 1], [0.85, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
            return (
              <div key={i} style={{
                display: "flex", alignItems: "center", gap: 24,
                background: "#2a2a2a", border: `2px solid ${C_LOG}44`,
                borderRadius: 20, padding: "24px 36px",
                opacity: appear, transform: `scale(${sc})`,
              }}>
                <span style={{
                  fontFamily: monoFont, fontFeatureSettings: MONO_NO_LIGA,
                  color: C_LOG, fontSize: 44, fontWeight: 900,
                  minWidth: 72, textAlign: "center",
                }}>{row.op}</span>
                <span style={{
                  fontFamily: uiFont, fontSize: 22, color: C_LOG, opacity: 0.65,
                  minWidth: 54,
                }}>{row.label}</span>
                <span style={{ color: "#555", fontSize: 28 }}>—</span>
                <span style={{
                  fontFamily: uiFont, fontSize: 30, color: "#d4d4d4",
                }}>{row.desc}</span>
              </div>
            );
          })}
...
export const JavaLogical: React.FC = () => (
  <AbsoluteFill style={{ background: "#1e1e1e" }}>
    ...
    <Sequence from={fromValues[2]} durationInFrames={VIDEO_CONFIG.andScene.durationInFrames}>
      <AndScene />
    </Sequence>
    <Sequence from={fromValues[3]} durationInFrames={VIDEO_CONFIG.orScene.durationInFrames}>
      <OrScene />
    </Sequence>
    <Sequence from={fromValues[4]} durationInFrames={VIDEO_CONFIG.notScene.durationInFrames}>
      <NotScene />
    </Sequence>
